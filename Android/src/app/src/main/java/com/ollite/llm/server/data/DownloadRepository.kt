...
import com.ollite.llm.server.worker.DownloadWorker
...
class DefaultDownloadRepository(
  ...
  private fun sendNotification(title: String, text: String, taskId: String, modelName: String) {
    ...
    val channelName = "Ollite download notification"
    ...
    // Download from global model manager. Open the global model manager screen.
      ...
      intent =
        Intent(Intent.ACTION_VIEW, "com.ollite.llm.server://global_model_manager".toUri())
      ...
      intent =
        Intent(
            Intent.ACTION_VIEW,
            "com.ollite.llm.server://model/$taskId/${modelName}".toUri(),
          )
          .apply { flags = Intent.FLAG_ACTIVITY_NEW_TASK }
    }
